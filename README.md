...
# INSTALLATION STEPS (for myself, to remember):

```bash
# 1. install dkms package
paru -S universal-pidff-dkms-git

# 2. go to src folder
cd /usr/src/universal-pidff-0.2.0.r12.ge27058c

# 3. manually replace hid-ids.h, hid-universal-pidff.c and dkms.conf

# 4. remove existing dkms
sudo dkms remove universal-pidff/0.2.0.r12.ge27058c --all 2>/dev/null || true

# 5. add updated dkms (symlink)
sudo dkms add -m universal-pidff -v 0.2.0.r12.ge27058c

# 6. build dkms
sudo dkms build -m universal-pidff -v 0.2.0.r12.ge27058c

# 7. install dkms
sudo dkms install -m universal-pidff -v 0.2.0.r12.ge27058c

# 8. check if it really worked (should be /lib/modules/6.xx.xx-cachyos/updates/dkms/hid_universal_pidff.ko.zst)
modinfo hid_universal_pidff | grep filename

# 9. replace current driver (may fail if it's not loaded on system startup)
sudo modprobe -r hid_universal_pidff 2>/dev/null

# 10. activate new driver
sudo modprobe hid_universal_pidff

# 11. add to autoload
echo "hid_universal_pidff" | sudo tee /etc/modules-load.d/hid-universal-pidff.conf

# 12. reboot
sudo reboot

# 13. check if it was started after system startup
lsmod | grep hid_universal_pidff

# 14. reconnect wheel USB

# 15. verify (should use hid-universal-pidff, NOT hid-generic)
sudo dmesg | tail -20 | grep -i "36e6\|universal\|pidff"
```
...
Make sure that all effects were played and the wheelbase reacted accordingly. 5. Switch back to terminal with running `journalctl` command, it should have a lot of debug messages
